...
export const apiDatabase: Record<string, APIData> = {
  ...
  'identity-document-verification': {
    ...
    longDescription: 'Our Identity Document Verification API uses cutting-edge OCR technology combined with machine learning algorithms to authenticate government-issued documents with 99.5% accuracy. The API supports over 200 document types from 195+ countries and includes advanced fraud detection capabilities.',
    responseTime: '<2s',
    accuracy: '99.5%',
    ...
    endpoints: [
      {
        ...
        description: 'Verify identity document authenticity and extract data with fraud detection'
      },
      {
        method: 'GET',
      ...
      },
      {
        ...
        description: 'Extract structured data from identity documents using OCR'
      }
    ],
    codeExamples: {
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'facial-recognition-match': {
    ...
    longDescription: 'Our Facial Recognition Match API provides state-of-the-art biometric verification by comparing live selfies with document photos. With 99.8% accuracy and advanced liveness detection, it prevents spoofing attacks and ensures genuine user verification.',
    responseTime: '<1s',
    accuracy: '99.8%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'address-verification': {
    ...
    longDescription: 'Our Address Verification API provides comprehensive address validation services across 200+ countries. It standardizes, validates, and enriches address data while checking against postal databases and utility records for maximum accuracy.',
    responseTime: '<500ms',
    accuracy: '98.2%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'phone-number-validation': {
    ...
    longDescription: 'Our Phone Number Validation API provides comprehensive phone number verification services including carrier lookup, line type detection, and risk scoring. It validates phone numbers across 200+ countries with real-time carrier data.',
    responseTime: '<300ms',
    accuracy: '97.9%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'email-verification': {
    ...
    longDescription: 'Our Email Verification API provides comprehensive email validation services including syntax validation, domain verification, mailbox existence checks, and risk scoring. It helps prevent fraud and ensures email deliverability.',
    responseTime: '<200ms',
    accuracy: '99.1%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'age-verification': {
    ...
    longDescription: 'Our Age Verification API provides comprehensive age validation services through document analysis, database lookups, and third-party data sources. It ensures compliance with age-restricted services and regulations.',
    responseTime: '<1.5s',
    accuracy: '98.7%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'company-registration-lookup': {
    ...
    longDescription: 'Our Company Registration Lookup API provides comprehensive business verification services across multiple jurisdictions. It validates company registration details, corporate structure, and regulatory compliance status in real-time.',
    responseTime: '<3s',
    accuracy: '99.1%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'ultimate-beneficial-owner': {
    ...
    longDescription: 'Our Ultimate Beneficial Owner API provides comprehensive UBO identification and verification services. It analyzes complex corporate structures to identify individuals who ultimately own or control a business entity.',
    responseTime: '<5s',
    accuracy: '98.7%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'sanctions-screening': {
    ...
    longDescription: 'Our Sanctions Screening API provides comprehensive screening against global sanctions lists including OFAC, EU, UN, and 40+ other regulatory bodies. It offers real-time screening with fuzzy matching and risk scoring.',
    responseTime: '<1s',
    accuracy: '99.9%',
...
response = requests.post(url, headers=headers, json=data)`
  ...
  'pep-screening': {
    ...
    longDescription: 'Our PEP Screening API provides comprehensive screening against Politically Exposed Person databases with relationship mapping, family connections, and risk scoring based on political exposure levels.',
    responseTime: '<2s',
    accuracy: '99.4%',
...
export const getAPIsByCategory = (category: string): APIData[] => {
  return Object.values(apiDatabase).filter(api => 
